...
dump_boot() {
  ...
  mkdir -p $ramdisk;
  cd $ramdisk;
  for i in $split_img/boot.img-ramdisk; do
    if [ -f "$i.gz" ]; then gunzip -c $i.gz | cpio -i;
    elif [ -f "$i.lzma" ]; then lzma -c $i.lzma | cpio -i;
    elif [ -f "$i.xz" ]; then xz -c $i.xz | cpio -i;
    elif [ -f "$i.lzo" ]; then lzop -c $i.lzo | cpio -i;
    else ui_print " "; ui_print "Unpacking ramdisk failed. Aborting..."; exit 1;
    fi;
  done;
...
write_boot() {
  ...
  $bin/mkbootimg --kernel $kernel --ramdisk /tmp/anykernel/ramdisk-new.cpio.gz --cmdline "$cmdline" --board "$board" --base $base --pagesize $pagesize --kernel_offset $kerneloff --ramdisk_offset $ramdiskoff $secondoff --tags_offset $tagsoff --output /tmp/anykernel/boot-new.img;
  if [ $? != 0 ]; then
